...
// -------- PÚBLICO --------
// GET /api/events?from=&to=&city=&country=&q=&page=&limit=
exports.getEventsPublic = async (req, res) => {
  try {
    ...
    if (q) filter.$text = { $search: q }; // crea índice de texto si lo usas (title/venue/description)
...
exports.getEventBySlugPublic = async (req, res) => {
  try {
    const doc = await Event.findOne({ slug: req.params.slug, isActive: true })
      .populate({ path: "lineup", select: "name slug photos.portraitUrl" });
...
// -------- ADMIN --------
// GET /api/admin/events?from=&to=&city=&country=&active=&q=&page=&limit=
exports.getEventsAdmin = async (req, res) => {
  try {
    const { city, country, active, q } = req.query;
    const { page, limit, skip } = getPagination(req.query);
    ...
    const [total, items] = await Promise.all([
      ...
      Event.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .select("title slug date venue city country isActive updatedAt")
        .lean()
    ]);
...
exports.createEvent = async (req, res) => {
  try {
    ...
    // Validar lineup (si llega) para evitar IDs inexistentes
    if (Array.isArray(payload.lineup) && payload.lineup.length > 0) {
      ...
      if (count !== payload.lineup.length) {
        return res.status(400).json({ message: "Some lineup artist IDs are invalid" });
      }
    }
    ...
    if (error.code === 11000) {
      return res.status(409).json({ message: "Duplicate key (slug must be unique)", error: error.keyValue });
    }
...
exports.updateEvent = async (req, res) => {
  try {
    ...
    // Si actualizas lineup, valida IDs
    if (Array.isArray(req.body.lineup)) {
      const count = await Artist.countDocuments({ _id: { $in: req.body.lineup } });
      if (count !== req.body.lineup.length) {
        return res.status(400).json({ message: "Some lineup artist IDs are invalid" });
      }
    }
